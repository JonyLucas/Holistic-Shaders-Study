Shader "Holistic/RaymarchCloudsCustom"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
                ...
                for (int z = 0; z <= 1; z++)
                {
                    ...
                    for (int y = 0; y <= 1; y++)
                    {
                        ...
                        for (int x = 0; x <= 1; x++)
                        {
                            ...
                            XValues[x] = random3d(cell, _WorldSpaceCameraPos - cell);
                        }
            ...
            float mapNoise(float3 seed)
            {
                ...
                f = noise3d(seed);
                seed *= f;
                f += 1 / 5 * noise3d(seed * sin(_Time) * 5);
                seed *= f * 1.5;
                f += 1 / 10 * noise3d(seed * cos(_Time) * 2);
                return NOISEPROC(f, p);
            }

            float mapNoise2(float3 seed)
            {
                float3 p = seed;
                float f;
                f = noise3d(seed);
                seed *= f;
                f += 1 / 5 * noise3d(seed * sin(_Time) * 2);
                seed *= f * 3;
                f += 1 / 3 * noise3d(seed * cos(_Time) * 2);
                return NOISEPROC(f, p);
            }
            ...
            fixed4 raymarch(float3 cameraPos, float3 viewDir, fixed4 bgcol, float depth)
            {
                ...
                MARCH(_Steps, mapNoise2, cameraPos, viewDir, bgcol, col, depth * 2, ct);
