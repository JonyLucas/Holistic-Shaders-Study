Shader "Holistic/RaymarchCloudsCustomSimplify"
{
    Properties
    {
        ...
        _SunPos ("Sun Direction", Vector) = (100,100,100)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                float4 sinCoeffs = float4(-0.16666667, 0.0083333310, -0.0001984090, 0.0000027526);
                float x2 = x * x;
                return x + x * x2 * dot(sinCoeffs, float4(1, x2, x2 * x2, x2 * x2 * x2));
            }

            // Approximate cos function
            float approxCos(float x)
            {
                float4 cosCoeffs = float4(-0.5, 0.041666638, -0.0013888378, 0.0000247609);
                float x2 = x * x;
                return 1.0 + x2 * dot(cosCoeffs, float4(1, x2, x2 * x2, x2 * x2 * x2));
            }

            // Approximate pow function
            float approxPow(float x, float y)
            {
            ...
            {
                return frac(sin(dot(value, seed)) * 12345);
            }
                ...
                for (int z = 0; z <= 1; z++)
                {
                    ...
                    for (int y = 0; y <= 1; y++)
                    {
                        ...
                        for (int x = 0; x <= 1; x++)
                        {
                            ...
                            XValues[x] = random3d(cell, _SunPos - cell);
                        }
            ...
            fixed4 integrate(fixed4 sum, float diffuse, float density, fixed4 bgcol, float t)
            {
                ...
                col.rgb *= lighting;
                col.rgb = lerp(col.rgb, bgcol, 1.0 - exp2(-0.003 * t * t));
                col.a *= 0.5;
                col.rgb *= col.a;
            ...
            float mapNoise(float3 seed)
            {
                float3 p = seed;
                float f;
                f = noise3d(seed);
                seed *= f;
                f += 1 / 5 * noise3d(seed * approxSin(_Time) * 5);
                seed *= f * 1.5;
                f += 1 / 10 * noise3d(seed * approxCos(_Time) * 2);
            ...
            fixed4 raymarch(float3 cameraPos, float3 viewDir, fixed4 bgcol, float depth)
            {
                ...
                MARCH(_Steps, mapNoise, cameraPos, viewDir, bgcol, col, depth, ct);
